...
void init_alarm(void)
{
  ...
  LPC_PINCON->PINSEL1 = LPC_PINCON->PINSEL1 & ~(0x03000000); 					//0xFCFFFFFF; 
	...
	LPC_GPIOINT->IO0IntEnR = LPC_GPIOINT->IO0IntEnR | (0x10000000);			//~(01<<28); //0x10000000; 
	LPC_GPIOINT->IO0IntEnF = LPC_GPIOINT->IO0IntEnF | (0x10000000);
	...
	LPC_GPIO2->FIOSET = 0x000000010;																		//Reset Alarmsummer
  	
	LPC_GPIO0->FIODIR = LPC_GPIO0->FIODIR & ~(1<<28);										//Pin28 (ALARMBUTTON) auf 0 => INPUT
	LPC_GPIO2->FIODIR = LPC_GPIO2->FIODIR | (1<<4);											//Pin4(SUMMER) auf 1 => OUTPUT 
...
{
	uint32_t alarm;
	alarm = (LPC_GPIO0->FIOPIN3 >> 4)& 0x01;
	return alarm;	
}
...
{
	if(alarm == 1){
		LPC_GPIO2->FIOCLR = 0x000000010;	//(1<<4);
	}else{
		LPC_GPIO2->FIOSET = 0x000000010;	//(1<<4);
	}
